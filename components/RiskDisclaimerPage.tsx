// components/RiskDisclaimerPage.tsx - Risk & Disclaimer page component
...
import { Info, FileText, Shield, AlertTriangle, TrendingDown, Ban } from 'lucide-react'
...
export function RiskDisclaimerPage() {
  ...
  const renderContentBlock = (block: ContentBlock, idx: number) => {
    switch (block.type) {
      ...
      case 'definition':
        return (
          <dl key={`def-${idx}`} className="space-y-2">
      ...
      case 'note':
        return (
          <div key={`note-${idx}`} className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-[12px] text-amber-100">
            {block.text}
          </div>
        )
      case 'paragraph':
      default:
        return (
          <p key={`p-${idx}`} className="text-slate-300/90">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 pb-16">
      ...
      <main className="pt-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-8 space-y-4">
              <div className="text-[11px] text-slate-500 flex items-center gap-1">
                ...
                <span className="text-slate-300">{t('breadcrumb.risk')}</span>
              </div>
            ...
            <div className="lg:col-span-4">
              <motion.div
                className="rounded-2xl bg-slate-950/90 border border-amber-500/40 p-4 flex flex-col gap-3"
                whileHover={{ y: -4, scale: 1.01 }}
                transition={{ type: 'spring', stiffness: 260, damping: 22 }}
              >
                <div className="flex items-center gap-2 mb-1">
                  <div className="h-8 w-8 rounded-full bg-amber-500/20 flex items-center justify-center border border-amber-500/40">
        ...
        <HomeSection className="pb-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <aside className="lg:col-span-3 space-y-3 text-[11px]">
              ...
              <div className="bg-slate-950/80 border border-amber-500/40 rounded-2xl p-3 space-y-1.5 text-slate-400">
                ...
                ? sections.map((section) => {
                    ...
                    return (
                      <motion.section
                        key={section.id}
                        id={section.id}
                        className={`bg-slate-950/80 rounded-2xl p-4 sm:p-5 space-y-3 ${theme.wrapper}`}
                        whileHover={{ y: -3 }}
        ...
        {/* Final CTA */}
        <HomeSection className="pb-12">
          <div className="bg-slate-950/90 border border-amber-500/40 rounded-2xl px-5 py-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
